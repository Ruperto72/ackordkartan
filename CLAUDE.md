...
`progRow(items, opts)` är den delade byggstenen för alla tre sorters ackordföljds-rader (inbyggda, under uppbyggnad, sparade): en rad diagram-mini-kort, valfritt med ‹/›-bläddring (`opts.scrollable`+`opts.onScroll`) och/eller en ✕ per kort (`opts.onRemove`), plus en spela-knapp och valfri ta bort-knapp (`opts.onDelete`).
