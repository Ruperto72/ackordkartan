...
- **Stilar** (`STYLE_PROGS`, `renderProgStyles()`+`renderStyleProgs()`) — en pill-väljare (samma mönster som familjefiltret) för tio musikstilar (blues, rock, punk, country, jazz, bossa nova, flamenco, spansk musik, klassisk musik, reggae). Varje stils ackord byggs direkt från grundton+kvalitet via `chordFromRootSfx(root,sfx)` (uppslaget i en ny `SFXMAP`, byggd av `QUALITIES`) — helt oberoende av `buildChords()`s skalfilter, så t.ex. en 12-takters blues kan använda I7/IV7/V7 även i lägen där de inte är diatoniska. `state.progStyle` (vald stil, `null`=ingen) nollställs **aldrig** vid rot- eller stämningsbyte — samma beteende som `state.family`. Bläddring återanvänder `progVoicings(chord, bassPc)`, som nu tar en valfri tvingande bas (samma mekanism som `state.slashBass` i Grepp-fliken) för de stilar vars ackordföljd innehåller ett slash-ackord (t.ex. Flamenco, Klassisk musik).
